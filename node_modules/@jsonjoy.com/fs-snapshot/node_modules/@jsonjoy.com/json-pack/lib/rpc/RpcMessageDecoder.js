...
class RpcMessageDecoder {
    decodeMessage(reader) {
        ...
        try {
            ...
            if (msgType === 0 /* RpcMsgType.CALL */) {
            ...
            else if (msgType === 1 /* RpcMsgType.REPLY */) {
                if (reader.size() < 4)
                    return (reader.x = startPos), undefined;
                const replyStat = reader.u32();
                if (replyStat === 0 /* RpcReplyStat.MSG_ACCEPTED */) {
                    const verf = this.readOpaqueAuth(reader);
                    if (!verf || reader.size() < 4)
                        return (reader.x = startPos), undefined;
                    const acceptStat = reader.u32();
                    let mismatchInfo;
                    if (acceptStat === 2 /* RpcAcceptStat.PROG_MISMATCH */) {
                        if (reader.size() < 8)
                            return (reader.x = startPos), undefined;
                ...
                else if (replyStat === 1 /* RpcReplyStat.MSG_DENIED */) {
                    if (reader.size() < 4)
                        return (reader.x = startPos), undefined;
                    const rejectStat = reader.u32();
                    let mismatchInfo;
                    let authStat;
                    if (rejectStat === 0 /* RpcRejectStat.RPC_MISMATCH */) {
                        if (reader.size() < 8)
                            return (reader.x = startPos), undefined;
                        const low = reader.u32();
                        const high = reader.u32();
                    ...
                    else if (rejectStat === 1 /* RpcRejectStat.AUTH_ERROR */) {
                        if (reader.size() < 4)
                            return (reader.x = startPos), undefined;
                        authStat = reader.u32();
                    }
