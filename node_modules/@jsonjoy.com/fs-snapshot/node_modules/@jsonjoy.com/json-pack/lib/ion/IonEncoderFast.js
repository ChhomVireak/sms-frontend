...
class IonEncoderFast {
    ...
    writeAny(value) {
        if (value instanceof ast_1.NullAstNode)
            this.writer.u8(0 /* TYPE_OVERLAY.NULL */ + 15);
    ...
    writeAnnotations(node) {
        ...
        if (node.len < 14)
            writer.u8(224 /* TYPE_OVERLAY.ANNO */ + node.len);
        else {
            writer.u8(224 /* TYPE_OVERLAY.ANNO */ + 14);
    ...
    writeBool(node) {
        this.writer.u8(16 /* TYPE_OVERLAY.BOOL */ + (node.val ? 1 : 0));
    }
    encodeUint(node) {
        const uint = node.val;
        if (!uint)
            this.writer.u8(32 /* TYPE_OVERLAY.UINT */);
        else if (uint <= 0xff)
            this.writer.u16(((32 /* TYPE_OVERLAY.UINT */ + 1) << 8) + uint);
        else if (uint <= 0xffff)
            this.writer.u8u16(32 /* TYPE_OVERLAY.UINT */ + 2, uint);
        else if (uint <= 0xffffff)
            this.writer.u32(((32 /* TYPE_OVERLAY.UINT */ + 3) << 24) + uint);
        else if (uint <= 0xffffffff)
            this.writer.u8u32(32 /* TYPE_OVERLAY.UINT */ + 4, uint);
        else {
            let lo = uint | 0;
            if (lo < 0)
                lo += 4294967296;
            ...
            if (uint <= 0xffffffffff) {
                this.writer.u16(((32 /* TYPE_OVERLAY.UINT */ + 5) << 8) + hi);
                this.writer.u32(lo);
            }
            else if (uint <= 0xffffffffffff) {
                this.writer.u8u16(32 /* TYPE_OVERLAY.UINT */ + 6, hi);
                this.writer.u32(lo);
            }
            else {
                this.writer.u16(((32 /* TYPE_OVERLAY.UINT */ + 7) << 8) + (hi >> 16));
    ...
    encodeNint(node) {
        const uint = -node.val;
        if (uint <= 0xff)
            this.writer.u16(((48 /* TYPE_OVERLAY.NINT */ + 1) << 8) + uint);
        else if (uint <= 0xffff)
            this.writer.u8u16(48 /* TYPE_OVERLAY.NINT */ + 2, uint);
        else if (uint <= 0xffffff)
            this.writer.u32(((48 /* TYPE_OVERLAY.NINT */ + 3) << 24) + uint);
        else if (uint <= 0xffffffff)
            this.writer.u8u32(48 /* TYPE_OVERLAY.NINT */ + 4, uint);
        else {
            let lo = uint | 0;
            if (lo < 0)
                lo += 4294967296;
            ...
            hi /= 4294967296;
            if (uint <= 0xffffffffff) {
                this.writer.u16(((48 /* TYPE_OVERLAY.NINT */ + 5) << 8) + hi);
                this.writer.u32(lo);
            }
            else if (uint <= 0xffffffffffff) {
                this.writer.u8u16(48 /* TYPE_OVERLAY.NINT */ + 6, hi);
                this.writer.u32(lo);
            }
            else {
                this.writer.u16(((48 /* TYPE_OVERLAY.NINT */ + 7) << 8) + (hi >> 16));
                this.writer.u16(hi & 0xffff);
                this.writer.u32(lo);
            }
        }
    ...
    writeFloat(node) {
        this.writer.u8f64(64 /* TYPE_OVERLAY.FLOT */ + 8, node.val);
    }
    writeVUint(num) {
        const writer = this.writer;
        if (num <= 0b1111111) {
    ...
    writeStr(node) {
        ...
        const writer = this.writer;
        if (length < 14)
            writer.u8(128 /* TYPE_OVERLAY.STRI */ + length);
        else {
            writer.u8(128 /* TYPE_OVERLAY.STRI */ + 14);
    ...
    writeBin(node) {
        ...
        const writer = this.writer;
        if (length < 14)
            writer.u8(160 /* TYPE_OVERLAY.BINA */ + length);
        else {
            writer.u8(160 /* TYPE_OVERLAY.BINA */ + 14);
            this.writeVUint(length);
        }
    ...
    writeArr(node) {
        ...
        const arr = node.val;
        if (arr === null) {
            writer.u8(176 /* TYPE_OVERLAY.LIST */ + 15);
            return;
        }
        const length = node.len;
        if (length < 14)
            writer.u8(176 /* TYPE_OVERLAY.LIST */ + length);
        else {
            writer.u8(176 /* TYPE_OVERLAY.LIST */ + 14);
            this.writeVUint(length);
        }
    ...
    writeObj(node) {
        ...
        const length = node.len;
        if (length < 14)
            writer.u8(208 /* TYPE_OVERLAY.STRU */ + length);
        else {
            writer.u8(208 /* TYPE_OVERLAY.STRU */ + 14);
            this.writeVUint(length);
        }
